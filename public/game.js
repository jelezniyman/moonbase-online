const socket = io(); // Инициализация сети
...
// Другие игроки (вместо NPC)
const otherPlayers = {};

const GRAVITY = 0.15; const JUMP_FORCE = 6; const MOVE_SPEED = 0.0035; const FRICTION = 0.85;
const keys = { left: false, right: false, up: false };

const stars = Array.from({length: 150}, () => ({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, size: Math.random() * 2, blink: Math.random() * 0.05 }));
...
socket.on('playerMoved', (playerData) => {
    if (otherPlayers[playerData.id]) {
        Object.assign(otherPlayers[playerData.id], playerData);
    }
});
...
socket.on('newProjectile', (p) => {
    projectiles.push(p); // Добавляем чужой снаряд
});
...
// Синхронизация своего положения на сервер 20 раз в секунду
setInterval(() => {
...
canvas.addEventListener('mousedown', (e) => {
    ...
    socket.emit('shoot', proj); // Отправляем выстрел по сети
});
...
joinBtn.addEventListener('click', () => {
    ...
    socket.emit('chatMessage', { name: 'СИСТЕМА', color: '#fff', text: `${player.name} присоединился к обороне Луны.` });
...
// --- ОТРИСОВКА ---
// Оставляем функции drawJointedLimbSegment, drawScienceStation, drawStandaloneFlag, drawGiantRocket, drawCharacter, drawLeaderboard без изменений с предыдущего шага.
...
function drawJointedLimbSegment(ctx, color, width, length, angleBase, angleOffset, isUpper = false) {
    ...
    if (isUpper) { ctx.roundRect(-width * 0.5, 0, width, length + width * 0.4, width * 0.4); } 
    else { ctx.roundRect(-width * 0.4, 0, width * 0.8, length, width * 0.3); }
...
function drawScienceStation(cx, cy, radius, angle) {
    ...
    ctx.fillStyle = '#ddd'; ctx.beginPath(); ctx.arc(0, 0, 70, Math.PI, 0); ctx.fill();
    ctx.fillStyle = 'rgba(100, 200, 255, 0.4)'; ctx.beginPath(); ctx.arc(0, 0, 60, Math.PI, 0); ctx.fill();
    ctx.fillStyle = '#114488'; ctx.fillRect(70, -40, 60, 15); ctx.fillRect(-130, -40, 60, 15);
    ctx.fillStyle = '#555'; ctx.fillRect(70, -30, 5, 30); ctx.fillRect(-75, -30, 5, 30);
    ctx.strokeStyle = '#aaa'; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.moveTo(0, -70); ctx.lineTo(0, -160); ctx.stroke();
    ctx.beginPath(); ctx.arc(0, -160, 30, Math.PI*0.1, Math.PI*0.9, true); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0, -160); ctx.lineTo(0, -180); ctx.stroke();
    ctx.fillStyle = 'red'; ctx.beginPath(); ctx.arc(0, -180, 4, 0, Math.PI*2); ctx.fill();
    ctx.restore();
}
...
function drawStandaloneFlag(cx, cy, radius, angle) {
    ...
    ctx.save(); ctx.translate(px, py); ctx.rotate(angle + Math.PI / 2);
    ctx.scale(2, 2); 
    ctx.fillStyle = '#999'; ctx.fillRect(-2, -120, 4, 120); 
    ctx.fillStyle = 'white'; ctx.fillRect(2, -120, 75, 15);
    ctx.fillStyle = 'blue'; ctx.fillRect(2, -105, 75, 15);
    ctx.fillStyle = 'red'; ctx.fillRect(2, -90, 75, 15);
    ctx.restore();
}
...
function drawGiantRocket(xOffset) {
    ...
    for(let i=0; i<3; i++) {
        let ty = 30 + i * 30; 
        ctx.fillStyle = 'orange'; ctx.beginPath(); ctx.moveTo(0, ty-12); ctx.lineTo(-120 - Math.random()*50, ty); ctx.lineTo(0, ty+12); ctx.fill();
        ctx.fillStyle = 'yellow'; ctx.beginPath(); ctx.moveTo(0, ty-6); ctx.lineTo(-60 - Math.random()*20, ty); ctx.lineTo(0, ty+6); ctx.fill();
        ctx.fillStyle = '#222'; ctx.fillRect(-15, ty-15, 35, 30);
    }
    ctx.fillStyle = '#f8f8f8'; ctx.beginPath(); ctx.moveTo(20, 10); ctx.lineTo(550, 10); ctx.quadraticCurveTo(800, 60, 800, 60); ctx.quadraticCurveTo(550, 110, 550, 110); ctx.lineTo(20, 110); ctx.closePath(); ctx.fill();
    ctx.strokeStyle = '#ccc'; ctx.lineWidth = 2;
    for (let i = 80; i < 500; i += 100) {
        ctx.beginPath(); ctx.moveTo(i, 10); ctx.lineTo(i, 110); ctx.stroke();
        ctx.fillStyle = '#bbb'; for (let r = 18; r < 110; r+= 18) { ctx.beginPath(); ctx.arc(i+6, r, 2, 0, Math.PI*2); ctx.fill(); }
    }
    ctx.fillStyle = '#cc0000'; ctx.beginPath(); ctx.moveTo(20, 10); ctx.lineTo(60, -30); ctx.lineTo(80, 10); ctx.fill(); ctx.beginPath(); ctx.moveTo(20, 110); ctx.lineTo(60, 150); ctx.lineTo(80, 110); ctx.fill();
    ctx.fillStyle = '#111'; ctx.font = 'bold 70px "Arial Black", sans-serif'; ctx.fillText("U S A", 300, 85);
    const flagX = 120; const flagY = 35; const flagW = 120; const flagH = 60;
    ctx.fillStyle = 'white'; ctx.fillRect(flagX, flagY, flagW, flagH);
    ctx.fillStyle = '#B22234'; for(let i=0; i<7; i++) { if(i%2===0) ctx.fillRect(flagX, flagY + i*(flagH/7), flagW, flagH/7); }
    ctx.fillStyle = '#3C3B6E'; ctx.fillRect(flagX, flagY, flagW*0.4, flagH*0.5);
    ctx.fillStyle = 'cyan'; ctx.strokeStyle = '#444'; ctx.lineWidth = 6; ctx.beginPath(); ctx.arc(550, 60, 20, 0, Math.PI*2); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.arc(640, 60, 30, 0, Math.PI*2); ctx.fill(); ctx.stroke(); 
    giantRocket.splats.forEach(s => {
        ctx.globalAlpha = s.life; ctx.fillStyle = s.color; ctx.beginPath();
        ctx.arc(s.x, s.y, s.r, 0, Math.PI*2); ctx.arc(s.x + s.r*0.8, s.y - s.r*0.5, s.r*0.6, 0, Math.PI*2); ctx.arc(s.x - s.r*0.5, s.y + s.r*0.9, s.r*0.7, 0, Math.PI*2); ctx.fill(); ctx.globalAlpha = 1.0;
    });
    ctx.restore();
}
...
function drawCharacter(charObj) {
    if (!charObj) return;
    let px = cx + Math.cos(charObj.angle) * charObj.distance;
    let py = cy + Math.sin(charObj.angle) * charObj.distance;
    ctx.save(); ctx.translate(px, py); ctx.rotate(charObj.angle + Math.PI / 2);
    ctx.save(); ctx.scale(charObj.direction, 1); 
    const torsoH = PLAYER_H * 0.35; const hipW = PLAYER_W * 0.5; const limbW = PLAYER_W * 0.3;  
    const thighL = PLAYER_H * 0.25; const calfL = PLAYER_H * 0.2; const armL = PLAYER_H * 0.2;   
    const armSwing = Math.cos(charObj.animTime) * 0.5; const legSwing = Math.sin(charObj.animTime) * 0.6; 
    const isJumping = !charObj.onGround; 
    ctx.fillStyle = charObj.color; ctx.beginPath(); ctx.roundRect(-PLAYER_W/2 - 5, -PLAYER_H * 0.75, 12, PLAYER_H * 0.5, 5); ctx.fill();
    const legY = -PLAYER_H * 0.25; 
    ctx.save(); ctx.translate(-hipW / 2, legY); let baseLegAngle = isJumping ? Math.PI / 2 - 0.4 : Math.PI / 2; 
    drawJointedLimbSegment(ctx, charObj.color, limbW, thighL, baseLegAngle, -legSwing, true); ctx.translate(0, thighL); 
    drawJointedLimbSegment(ctx, charObj.color, limbW, calfL, baseLegAngle, legSwing > 0 ? legSwing*1.2 : legSwing*0.2, false); ctx.restore();
    ctx.fillStyle = charObj.color; ctx.beginPath(); ctx.roundRect(-PLAYER_W/2, -PLAYER_H * 0.65, PLAYER_W, torsoH + PLAYER_H * 0.15, hipW/2); ctx.fill(); 
    ctx.save(); ctx.translate(hipW / 2, legY); drawJointedLimbSegment(ctx, charObj.color, limbW, thighL, baseLegAngle, legSwing, true); 
    ctx.translate(0, thighL); drawJointedLimbSegment(ctx, charObj.color, limbW, calfL, baseLegAngle, legSwing < 0 ? legSwing*0.2 : legSwing*1.2, false); ctx.restore();
    const armY = -PLAYER_H * 0.6; const armBaseAngle = isJumping ? Math.PI / 2 + 0.6 : Math.PI / 2; 
    ctx.save(); ctx.translate(-PLAYER_W * 0.4, armY); drawJointedLimbSegment(ctx, charObj.color, limbW * 0.8, armL, armBaseAngle, -armSwing, true); ctx.translate(0, armL); drawJointedLimbSegment(ctx, charObj.color, limbW * 0.8, armL, armBaseAngle, -armSwing * 0.5, false); ctx.restore();
    ctx.save(); ctx.translate(PLAYER_W * 0.4, armY); drawJointedLimbSegment(ctx, charObj.color, limbW * 0.8, armL, armBaseAngle, armSwing, true); ctx.translate(0, armL); drawJointedLimbSegment(ctx, charObj.color, limbW * 0.8, armL, armBaseAngle, armSwing * 0.5, false); ctx.restore();
    const headRadius = PLAYER_W / 2 + 5; const headY = -PLAYER_H * 0.65;
    ctx.save(); ctx.translate(0, headY);
    ctx.fillStyle = '#aaa'; ctx.beginPath(); ctx.roundRect(-headRadius*0.9, -5, headRadius*1.8, 10, 3); ctx.fill();
    ...
    ctx.fillStyle = 'white'; ctx.font = 'bold 32px Arial'; ctx.textAlign = 'center'; ctx.fillText(charObj.name, 0, -PLAYER_H - headRadius - 20);
    ctx.restore();
}

function drawLeaderboard() {
    ctx.save(); ctx.fillStyle = "rgba(0, 0, 0, 0.6)"; ctx.beginPath(); ctx.roundRect(canvas.width - 230, canvas.height - 310, 220, 300, 10); ctx.fill();
    ctx.fillStyle = "#aaaaaa"; ctx.font = "16px 'Courier New', Courier, monospace"; ctx.textAlign = "right"; 
    let startX = canvas.width - 25; let startY = canvas.height - 280; 
    ...
    for (let i = 0; i < 13; i++) {
        ...
        ctx.fillText(`${i + 1}. ${scoreText}`, startX, startY + 25 + (i * 15)); 
    }
    ctx.fillStyle = "#00FF00"; ctx.font = "bold 18px 'Courier New', Courier, monospace"; ctx.fillText(`Ваш счет: ${currentScore}`, startX, startY + 40 + (13 * 15));
    ctx.restore();
}
...
function draw() {
    ...
    const earthX = canvas.width * 0.75; const earthY = 250; const earthRadius = 100; 
    ...
    rovers.forEach(r => { 
        let px = cx + Math.cos(r.angle) * moonRadius; let py = cy + Math.sin(r.angle) * moonRadius;
        ctx.save(); ctx.translate(px, py); ctx.rotate(r.angle + Math.PI / 2); ctx.fillStyle = '#e0aa00'; ctx.fillRect(-20, -20, 40, 20); ctx.fillStyle = '#333'; ctx.beginPath(); ctx.arc(-10, 0, 8, 0, Math.PI*2); ctx.arc(10, 0, 8, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle = '#888'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(15, -20); ctx.lineTo(15, -45); ctx.stroke(); ctx.fillStyle = 'red'; ctx.beginPath(); ctx.arc(15, -45, 3, 0, Math.PI*2); ctx.fill(); ctx.restore();
    });

    projectiles.forEach(p => { 
        ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(Math.atan2(p.vy, p.vx)); 
        if (p.type === 'tomato') { ctx.fillStyle = '#e60000'; ctx.beginPath(); ctx.arc(0, 0, 8, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#009900'; ctx.fillRect(-2, -10, 4, 5); } 
        else if (p.type === 'banana') { ctx.fillStyle = '#ffe135'; ctx.beginPath(); ctx.arc(0, 0, 10, 0.2, Math.PI); ctx.fill(); } 
        else if (p.type === 'egg') { ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.ellipse(0, 0, 8, 6, 0, 0, Math.PI*2); ctx.fill(); }
        ctx.restore();
    });
...
window.addEventListener('resize', () => { canvas.width = window.innerWidth; canvas.height = window.innerHeight; cx = canvas.width / 2; cy = canvas.height / 2 + 250; });
